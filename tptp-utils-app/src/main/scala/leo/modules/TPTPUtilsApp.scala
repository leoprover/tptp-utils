...
object TPTPUtilsApp {
      ...
      try {
        ...
        val result = command.get match {
          case Parse =>
            ...
            generateSZSResultWithPrefix(result.pretty, "Success", "")
          case Reparse =>
            val parsedInput = parseTPTPFile(infile.get)
            val json: String = ParseTree(parsedInput)
            generateSZSResultWithPrefix(json, "Success", "LogicalData")
          case Transform(goal) =>
            val parsedInput = parseTPTPFile(infile.get)
            val transformed = SyntaxTransform(goal, parsedInput)
            generateSZSResultWithPrefix(tptpProblemToString(transformed), "Success", "ListOfFormulae")
          case Downgrade(goal) =>
            try {
              val parsedInput = parseTPTPFile(infile.get)
              ...
              case e: IllegalArgumentException => generateSZSResultWithPrefix("", "InputError", "", e.getMessage)
            }
          case Lint =>
            val parsedInput = parseTPTPFile(infile.get)
            val lint = Linter(parsedInput)
            val result = lint.mkString("\n")
            generateSZSResultWithPrefix(result, "Success", "LogicalData")
          case Import(from) =>
            val result = tptputils.Import(infile.get, from)
            generateSZSResultWithPrefix(tptpProblemToString(result), "Success", "ListOfFormulae")
          ...
          case Fragment =>
            ...
            generateSZSResultWithPrefix(tptpProblemToString(resultProblem), "Success", "ListOfFormulae") ++
            generateSZSStatus("Fragment", overallFragmentClass.toString)
        }
  ...
  private[this] final def generateSZSResultWithPrefix(result: String, szsStatus: String, szsDatatype: String, extraTSTPMessage: String = ""): String = {
    ...
    if (result.nonEmpty) {
      ...
      if (tstpOutput) {
        generateSZSStatus(szsStatus, extraTSTPMessage)
      }
      sb.append(generateSZSOutput(result, szsDatatype))
    }
    sb.toString()
  }
  private[this] final def generateSZSStatus(szsStatus: String, extraTSTPMessage: String = ""): String = {
    if (extraTSTPMessage == null || extraTSTPMessage.isEmpty) s"% SZS status $szsStatus for $inputFileName\n"
    else s"% SZS status $szsStatus for $inputFileName : $extraTSTPMessage\n"
  }
