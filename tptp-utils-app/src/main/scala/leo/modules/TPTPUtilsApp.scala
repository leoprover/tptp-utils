...
object TPTPUtilsApp {
  ...
  final val version: String = "1.4.1"
      ...
      try {
        ...
        val result = command.get match {
          case Parse =>
            ...
            generateSZSResult(result.pretty, "Success", "", withPrefix = true)
          case Reparse =>
            val parsedInput = parseTPTPFile(infile.get)
            val json: String = ParseTree(parsedInput)
            generateSZSResult(json, "Success", "LogicalData", withPrefix = true)
          case Transform(goal) =>
            val parsedInput = parseTPTPFile(infile.get)
            val transformed = SyntaxTransform(goal, parsedInput)
            generateSZSResult(tptpProblemToString(transformed), "Success", "ListOfFormulae", withPrefix = true)
          case Downgrade(goal) =>
            try {
              val parsedInput = parseTPTPFile(infile.get)
              ...
              case e: IllegalArgumentException => generateSZSResult("", "InputError", "", withPrefix = true, e.getMessage)
            }
          case Lint =>
            val parsedInput = parseTPTPFile(infile.get)
            val lint = Linter(parsedInput)
            val result = lint.mkString("\n")
            generateSZSResult(result, "Success", "LogicalData", withPrefix = true)
          case Import(from) =>
            val result = tptputils.Import(infile.get, from)
            generateSZSResult(tptpProblemToString(result), "Success", "ListOfFormulae", withPrefix = true)
  ...
  private[this] final def generateSZSResult(result: String, szsStatus: String, szsDatatype: String, withPrefix: Boolean, extraTSTPMessage: String = ""): String = {
    ...
    if (result.nonEmpty) {
      ...
      if (tstpOutput) {
        sb.append(generateSZSStatus(szsStatus, extraTSTPMessage))
      }
